...
//                lock.hashCode();
        ...
        Thread.sleep(500);
        printf("9_重量级锁结束：" +getLongBinaryString(unsafe.getLong(lock, 0L)));


        synchronized (lock) {
            // 对象头为：指向线程栈中的锁记录指针
            printf("10_轻量级锁：" + getLongBinaryString(unsafe.getLong(lock, 0L)));
        }
        new Thread(() -> {
            synchronized (lock) {
                printf("11_轻量级锁：" +getLongBinaryString(unsafe.getLong(lock, 0L)));
            }
        }).start();
        Thread.sleep(1000);
    }
